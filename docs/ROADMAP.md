...
| **LOCALCI-4** — Forgejo as primary forge 🚧 (2026-09-16, ADR-028) | The self-hosted Forgejo runs the whole pipeline from `.forgejo/workflows/` (a held copy — R80) on the desk's WSL + Windows runners; deploys publish to `deploy/*` on GitHub, prod is a manual dispatch; GitHub CI runs only on a deliberate push. Supersedes LOCALCI-1 for this repo. | all of it — no minutes billed |
