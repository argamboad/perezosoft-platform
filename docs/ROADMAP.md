...
- **Multi-household membership** (`MULTIHOME`) — **decided: deferred (ADR-029, 2026-09-16)**; one user in
...
| **LOCALCI-4** — Forgejo as primary forge 🚧 (2026-09-16, ADR-028) | The self-hosted Forgejo runs the whole pipeline from `.forgejo/workflows/` (a held copy — R80) on the desk's WSL + Windows runners; pushes run gates + builds, the native smokes and the deploys are dispatch-only (a deploy fast-forwards GitHub's branch, so GitHub re-deploys too); GitHub CI otherwise runs only on a deliberate push. Supersedes LOCALCI-1 for this repo. | all of it — no minutes billed |
