...
| **Safe areas / status bar / edge-to-edge** | ✅ **fixed (2026-07-07)** | It failed on an Android 16 emulator exactly as predicted: the enforced edge-to-edge laid the WebView under the transparent status bar, and the clock/status icons drew over the app header. The CSS `env(safe-area-inset-top)` guard can't be relied on — the Android 16 WebView of 2026-07 resolved env() to 0 (keep it for the iOS notch). **Fixed natively:** `MainActivity` pads the content view by the status-bar/cutout inset via `ViewCompat.SetOnApplyWindowInsetsListener`. Bottom stays edge-to-edge (gesture pill floats; 3-button nav draws its own scrim). Verified live on the API 36 emulator (screencap: content below the bar, green strip behind the clock). **Amended 2026-09-16** (found downstream on a Pixel 10 Pro XL emulator): current WebViews DO report the status-bar inset, so the header's `env()` added it a second time — an empty ~55px band. The listener now returns the insets with status bars and cutout spent, so `env(safe-area-inset-top)` is 0 on every Android WebView. The strip was also a hard-coded sage (`#6B8A72`) with light icons in every theme; it now follows the page — `SystemBarThemeSync` → `ISystemBarTheme` (Android only) → `SystemBarColors`: the header's `--bs-primary`, or `--app-bg` on the headerless sign-in/boot screens, icons chosen by the strip's luminance (`NativeChromeGateTests` holds the colours to app.css). |
