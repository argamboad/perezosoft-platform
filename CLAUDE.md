...
| `docs/QA_TEST_PLAN.md` | Manual QA plan — step-by-step tests across web + all four native platforms (156 cases: smoke + regression + §14a v3-audit adversarial/tenant-isolation + §14c pre-launch gates + §13c native release checklist) |
...
| `docs/stories/async-jobs.md` | epic `JOBS` ✅ COMPLETE — outbox+dispatcher, inbox, scheduler (ADR-007); JOBS-4 (2026-09-16) file attachments on `IEmailSender` (≤ 10 MiB total, validated before enqueue — pass the `CancellationToken` by name) |
...
| `docs/DEPLOYMENT.md` | Deployment runbook (DEPLOY-2/3) — Render + Neon + Brevo free-tier bring-up; `Dockerfile` + `render.yaml` reference; required env incl. the Production Stripe-key guard; §6 CI-gated auto-deploy (GitHub); **§10 Forgejo as the primary forge — remotes, runners, secrets, `deploy/*` branches (LOCALCI-4)** |
| `docs/stories/localci.md` | epic `LOCALCI` 🚧 IN PROGRESS — **LOCALCI-4 🚧 (2026-09-16, ADR-028): Forgejo is the primary forge (`origin`; GitHub = `github` remote, pushed to on purpose) and runs the FULL pipeline from `.forgejo/workflows/` on the desk's runners (`ubuntu-latest` + `ubuntu-host-ports` in WSL, `windows-latest` host mode; Apple legs wait for `vars.CI_MACOS_RUNNER`); deploys push the commit to `deploy/*` on GitHub (no GitHub Actions run) then fire the Render hook; prod = manual dispatch on main; `ForgejoCiParityTests` (R80) holds the two workflow copies together — edit BOTH, runbook `DEPLOYMENT.md` §10; supersedes LOCALCI-1 here.** **LOCALCI-3 ✅ shipped 2026-09-11** (built downstream in jigger-jot, ported here and to vuelto the same day): `changes` computes `code`/`native`/`docs` from the diff and every non-deploy job gates on it, so a docs-only push stops billing ~30 minutes for markdown. **`secret-scan` and `qa-artifacts` NEVER gate on code** — a docs commit can leak a credential, and editing the QA plan without regenerating the PDFs is the only way to break the artifacts check, so gating it would blind CI to exactly what it catches (a test enforces both halves). Fails OPEN on an unknown diff base. The Apple SMOKE (87 billed min, macOS at 10×) moves to a Monday cron + dispatch until `vars.CI_MACOS_RUNNER` is set; the Apple BUILD still runs per develop push, so compile rot is caught within one merge. ⚠️ A green develop run is NOT a green Apple smoke — see QA §13c before shipping a native client. Original plan (2026-09-08) — local + self-hosted CI, pick-up-ready: LOCALCI-1 variable-driven `runs-on` (`vars.CI_{LINUX,WINDOWS,MACOS}_RUNNER` → self-hosted, hosted fallback; deploys stay hosted; R77 tripwire; runner install runbook → DEPLOYMENT §10; ADR-025 draft in the file) · LOCALCI-2 land `ci-local.ps1` (extract from the STALE unpushed `ci/local-gates` branch, never merge it) + R78 pin/gate-list tripwire · LOCALCI-3 paths gate for every non-deploy job + weekly Apple smoke in hosted mode. Measured: ≈190 billed min per develop code push, ≈157 of them macOS/Windows |
...
| `docs/stories/gates.md` | epic `GATES` ✅ COMPLETE on the platform (2026-09-11; downstream port to vuelto + jigger-jot still open) — **pre-launch gates**, both deployment config, both cleared on launch day (ADR-027): GATES-1 `Billing:Enabled` default-off (billing controllers + provider webhook removed from the application model ⇒ 404; client hides the link and refuses `/billing`; everyone resolves Free via `PlanCatalog.Get`'s fail-closed fallback; Free seats **3→5**; billing-off seat-limit/full-household copy so nothing offers an upgrade that has no provider account behind it) · GATES-2 signup green list (`Signup:AllowedEmails`/`AllowedDomains`, empty = open) — the list decides **who may found a household**; a valid pending invitation admits its addressee iff that invitation's tenant **owner** is green-listed (owner, not inviter, so a non-listed admin can still invite into their owner's household). Gates **account creation only, never sign-in**, at the single choke point `UserService.CreateUserWithTenantAsync`; deliberately does NOT fire at magic-link/OTP **issue** time (that would be a user-enumeration oracle) |
